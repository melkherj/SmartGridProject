...
good_dates = set(date.strip() for date in open(os.path.join(summary_data_path,'good_dates.txt'),'r').readlines())

for line in sys.stdin:
    tag,date = line.split('^',2)[:2]
    if (tag.strip() in good_tags) and (date.strip() in good_dates):
        print line.strip()
